...
- Build an inset with the **narrowest `EdgeInsets` constructor** that describes it: all four sides equal → `EdgeInsets.all` (`EdgeInsets.zero` when they are all zero); left equals right and top equals bottom → `EdgeInsets.symmetric`, leaving out a zero pair; anything else → `EdgeInsets.only`, naming the non-zero sides. **Never `EdgeInsets.fromLTRB`**: four positional numbers read only with the left-top-right-bottom order in mind, and a mockup's CSS lists the sides in another order (top-right-bottom-left), so a side swapped in transit goes unnoticed. Performance does not enter the choice — every one of these is `const` and builds the same object. The same holds for `EdgeInsetsDirectional` (`only(start:, end:)`, not `fromSTEB`). Don't assemble an inset with `copyWith` or `+`: neither is `const`, so the object is rebuilt on every `build` and the widget holding it loses `const` too.
